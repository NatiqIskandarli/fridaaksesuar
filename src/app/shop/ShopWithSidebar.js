...
import { getAllProductsBySub } from "@/http/product";


const ShopWithSidebar = ({searchParams}) => {
    ...
    const categoryHandler = (cateSelect) => {
        const cateFilterProduct = filterProduct.filter((data) =>(slugify(data.subCategoryId) === cateSelect));
    ...
    const priceRangeHandler = (rangeSelect) => {
        const getPriceData = filterProduct.filter(data => data.price <= rangeSelect);
    ...
    const priceRangeData = [
     ...
     useEffect(()=>{
        const fetchProd = async () =>{
            try {
                const resultProd = await getAllProductsBySub(searchParams.alt)
                ...
                <div className="col-lg-3">
                    <div className="axil-shop-sidebar">
                        ...
                        <div className={`toggle-list product-categories ${cateToggle ? "active" : ""}`}>
                            <h6 onClick={() => setcateToggle(!cateToggle)} className="title"> Kateqoriya</h6>
                            {cateToggle && 
                        ...
                        <div className={`toggle-list product-price-range ${priceRangeToggle ? "active" : ""}`}>
                            <h6 onClick={() => setpriceRangeToggle(!priceRangeToggle)} className="title">Qiymət</h6>
                            {priceRangeToggle && 
                            <div className="shop-submenu">
                                <ul>
                        ...
                        <button className="axil-btn btn-bg-primary" onClick={() => productFilterReset()} >Təmizlə</button>
                    </div>
                </div>
                ...
                <div className="col-lg-9">
                    <div className="row row--15">
                        {filterProduct.length > 0 ? filterProduct.slice(0, productShow).map((data) => (
                            <div className="col-xl-4 col-sm-6" key={data.productId}>
                                <ProductOne product={data} pClass="mb--30" />
                            </div>
                        )) : <h4 className="text-center pt--30">No Product Found</h4>}
                    </div>
                    <div className="text-center pt--20">
                        <button className={`axil-btn btn-bg-lighter btn-load-more ${filterProduct.length < productShow ? "disabled" : ""}`} onClick={ProductShowHandler}>{filterProduct.length < productShow ? "No More Data" : "Çoxalrt"}</button>
                    </div>
                </div>
            </div>
